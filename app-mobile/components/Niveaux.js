...
const Niveaux = () => {
  ...
  useEffect(() => {
    if (leagues) {
      axios
        .get(`https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-name/${leagues}?api_key=${API_KEY}`)
        .then((response) => {
    ...
    <View style={styles.container} >
      ...
      {Object.keys(playerData).length !== 0 ? (
        <>
          ...
          />
          <Text style={styles.txt}>Summoner level {playerData.summonerLevel}</Text>
        </>
      ) : (
        <></>
      )}
...
<View>
      {rank ? (
        <View>
          <Text style={styles.txt}>Vous etes classé {rank.tier} {rank.rank}</Text>
          <Text style={styles.txt}>{rank.wins} victoires / {rank.losses} défaites</Text>
        </View>
      ) : (
        <View>
